...
export const showGameOver = (message, title = "Fim de Jogo!", buttonOptions = {}) => {
    ...
    // Grant achievements on game over (NON-PVP ONLY - PvP is handled by server)
    if (gameState && !gameState.isPvp && action === 'restart') {
        const player1 = gameState.players['player-1'];
        const player1Won = message.includes(player1?.name || '---');
